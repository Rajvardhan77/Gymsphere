...
def create_app() -> Flask:
    ...
    def dashboard():
        ...
        try:
            ...
            hydration_goal = 3000 # 3 Liters default
            ...
            active_plan = UserPlan.query.filter(
                UserPlan.user_id == current_user.id,
                UserPlan.end_date >= today_date
            ).order_by(UserPlan.created_at.desc()).first()
            
            if active_plan:
                streak_data = compute_streaks(current_user.id, active_plan.id)
            ...
            hydration_current = 0
            hydration_goal = 3000
            sleep_data = {"hours": 0, "quality": "-"}
            user_badges = []
            current_streak = 0
            
            # Fallbacks for critical UI components
            if not workout:
                workout = {
                    "frequency": 3,
                    "exercises": [],
