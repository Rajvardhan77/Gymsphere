...
def schedule_tomorrow_plan_notification(user, plan_id):
    """
    Check tomorrow's plan and notify user.
    """
    from models import DailyPlanEntry
    tomorrow = datetime.utcnow().date() + timedelta(days=1)
    
    entry = DailyPlanEntry.query.filter_by(plan_id=plan_id, date=tomorrow).first()
    if not entry:
        return

    # De-duplicate
    title = "Tomorrow's Plan Ready 📅"
    exists = Notification.query.filter_by(user_id=user.id, title=title, is_read=False).first()
    if exists: return

    if entry.is_exercise_day:
        workout_name = " & ".join([ex['name'] for ex in (entry.exercise_payload or [])[:2]])
        msg = f"Tomorrow: {workout_name}... Get ready to crush it!"
    else:
...
def schedule_morning_reminder(user):
    """
    ...
    msgs = [
        "New day, new goals. Log your breakfast to start strong!",
        "Consistency is key. Have you planned your workout?",
        "Don't forget to drink water today!",
        "Your future self will thank you. Let's get moving."
    ]
    import random
    create_notification(user, title, random.choice(msgs), type="reminder")


def check_streak_and_notify(user, current_streak):
    """
    Celebrate streaks or warn about breaking them.
    """
    if current_streak > 0 and current_streak % 3 == 0:
        title = f"{current_streak} Day Streak! 🔥"
        exists = Notification.query.filter_by(user_id=user.id, title=title).first()
        if not exists:
            create_notification(user, title, "You are on fire! Keep the momentum going.", type="streak_alert")



def weekly_progress_check(user: User) -> Optional[Notification]:
    """
    Check weekly progress and generate notification messages.
    """
    ...
    try:
        now = datetime.utcnow()
        last_notification = (
            user.notifications.order_by(Notification.created_at.desc()).first()
            if hasattr(user, "notifications") and user.notifications else None
        )
        last_check = last_notification.created_at if last_notification else (user.created_at if hasattr(user, "created_at") else now - timedelta(days=8))
        
        if (now - last_check).days < 7: return None
        
        # Simple progress check
        create_notification(user, "Weekly Check-in", "Don't forget to weigh in and update your progress logs!", type="system")
        
    except Exception as e:
        print(f"Error in weekly_progress_check: {e}")
    return None
...
def compute_streaks(user_id: int, plan_id: int) -> Dict:
    ...
    today = datetime.utcnow().date()
    entries = DailyPlanEntry.query.filter_by(plan_id=plan_id).filter(DailyPlanEntry.date <= today).order_by(DailyPlanEntry.date.desc()).all()
    ...
    for entry in entries:
        ...
        is_done = False
        if entry.is_exercise_day:
            if entry.is_exercise_completed and entry.is_diet_completed:
                is_done = True
        else:
        ...
        if is_done:
            ...
            # If it's today and not done, it doesn't break the streak from yesterday
            if entry.date == today:
                continue
            else:
                break
